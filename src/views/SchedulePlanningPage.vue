<template>
  <div class="schedule-planning">
    ...
    <div class="stepper">
      <div v-for="(step, idx) in steps" :key="step.label" :class="['step', {active: currentStep === idx}]">
    ...
    <Card v-if="currentStep === 1" class="step-card">
    ...
    <Card v-if="currentStep === 2" class="step-card">
      <template #title>Исключения</template>
      <template #content>
        <div class="form-block">
          <label>Исключить даты:</label>
          <Calendar v-model="excludeDates" selection-mode="multiple" placeholder="Выберите даты" />
          <Button label="Добавить" @click="addExcludeDates" :disabled="!excludeDates.length" />
          <div class="excluded-list">
            <span v-for="(date, idx) in excludedDates" :key="date">
              {{ date }} <Button icon="pi pi-times" text @click="removeExcludeDate(idx)" />
            </span>
          </div>
        </div>
        <div class="step-actions">
          <Button label="Назад" class="p-button-text" @click="prevStep" />
          <Button label="Далее" @click="generateLessonsAndNext" :disabled="!canGenerate" />
        </div>
      </template>
    </Card>
    ...
    <Card v-if="currentStep === 3" class="step-card">
      ...
      <template #content>
        ...
        <DataTable v-if="!isMobile" :value="lessons" edit-mode="row" dataKey="date" class="mb-2">
          ...
          <Column field="lessonNumber" header="Пара" :editor="lessonNumberEditor" />
        ...
        <div class="step-actions">
          ...
          <Button label="Очистить всё" class="p-button-danger" @click="clearAll" />
        </div>
      </template>
    </Card>
...
const filteredSubjects = computed(() => {
  ...
  const assignmentsForGroup = teachingAssignments.value.filter(a => a.group.id === selectedGroup.value);
  const subjectIds = [...new Set(assignmentsForGroup.map(a => a.subject.id))];
...
const filteredAssignments = computed(() => {
  ...
  return teachingAssignments.value.filter(a =>
    a.group.id === selectedGroup.value && a.subject.id === selectedSubject.value
  );
});
